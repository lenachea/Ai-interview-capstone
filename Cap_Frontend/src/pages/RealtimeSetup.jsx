...
export default function RealtimeSetup() {
  ...
  const startVolumeViz = useCallback((stream) => {
    try {
      ...
      analyser.smoothingTimeConstant = 0.85  // 높을수록 반응이 느리고 부드러움
      ...
      function tick() {
        ...
        const level = Math.min(1, rms / 160)   // 0~1 (높을수록 덜 민감)
            ...
            <div className={styles.micCard}>
              ...
              {micStatus === 'granted' ? (
                <div className={styles.volumeWrap}>
                  {/* width=600 height=100 → CSS로 반응형 조절 */}
                  <canvas ref={canvasRef} className={styles.volumeCanvas} width={600} height={100} />
